...
class ImageData(object):
    ...
    def __init__(self, filename=None, task=True, image=None, annotation=None, scale=None, label=None, masks_and_category=None):
        ''' filename is for saving file
            _task is for knowing the task is object detection (True) or image classification (False)
        ...
        self._task = task #TODO: CHANGE THIS to self._task #Underscore everything thats internal
        self.scale = scale 
                ...
                for i in range(len(self.filename)):
                    ...
                    if masks is not None:
                        ...
                        for j in range(len(self.masks)):
                            reshape_image = self.apply_mask(
                                reshape_image, self.masks[j], color_cat_dict[self.categories[j]], alpha)
                        else:
        ...
        if path is None:
            ...
            if isinstance(self.filename, list):
                ...
                for i in range(len(self.filename)):
                    ...
                    print(self.filename[i]," is saving to ",save_img_path," ...")
            else:
                skimage.io.imshow(self.image)
                skimage.io.show()
                os.makedirs(path, exist_ok=True)
